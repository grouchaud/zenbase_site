document.write('\
<footer id="footy">\
	<div class="row clearfix">\
		<div class="col-xs-12 text-center tagline-footer">\
			<h1>Contactez-moi dès maintenant !\
			</h1>\
		</div>\
	</div>\
	<div class="raw">\
		<div class="hidden-xs col-sm-12">\
			<div class="cta text-center">\
				<p><strong>Contact ici</strong></p>\
				<div class="col-sm-6">\
					<input type="text" name="location" id="INPUT_1" placeholder="Votre email" />\
				</div>\
				<div class="col-sm-6">\
					<input type="submit" name="commit" value="Essayez maintenant" id="INPUT_2" />\
				</div>\
			</div>\
		</div>\
		<div class="hidden-md hidden-sm hidden-lg col-xs-12">\
			<div class="ctaxs text-center">\
				<p><strong>Contact ici</strong></p>\
				<div class="col-xs-12">\
					<input type="text" name="location" id="INPUT_1xs" placeholder="Votre email" />\
				</div>\
				<div class="col-xs-12">\
					<input type="submit" name="commit" value="Essayez maintenant" id="INPUT_2xs" />\
				</div>\
			</div>\
		</div>\
	</div>\
	<div class="container">\
		<div class="row index-row hidden-xs">\
			<div class="col-sm-4 index">\
				<h5><p>Product</p></h5>\
					<p> <a class=menuindex href="price.html">Prix</a> </br>\
					<a class=menuindex href="">Clients</a> </br>\
					<a class=menuindex href="product.html">Product Tour</a> </p>\
			</div>\
			<div class="col-sm-4 index">\
				<h5>Company</h5>\
				<p> <a class=menuindex href="">Mon Histoire</a> </br>\
				<a class=menuindex href="contact.html">Contact</a> </br>\
				<a class=menuindex href="">Partners</a> </br>\
			</div>\
			<div class="col-sm-4 index">\
				<h5>Content</h5>\
				<p> <a class=menuindex href="">Blog</a> </p>\
			</div>\
		</div>\
	</div>\
	<div class="row">\
		<div class="col-xs-12 text-center">\
			<p>\
				<a class="fa fa-linkedin fa-2x" href="https://www.linkedin.com/in/jacques-rouchaud-27128342/" target="_blank"></a>\
				<a class="fa fa-envelope fa-2x" href="mailto:jr@bogastar.com?Subject=Hello%20again" target="_blank"></a>\
			</p>\
		</div>\
	</div>\
    <!--<a href="https://twitter.com/intent/tweet?text=@zenbase_+Type+your+question+here&amp;source=webclient" >Tweet Us</a>-->\
</footer>\
');